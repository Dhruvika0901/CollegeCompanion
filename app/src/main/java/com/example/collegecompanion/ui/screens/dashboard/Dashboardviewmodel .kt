...
data class AttendanceDashboardSummary(
    ...
    // Exposed so DashboardScreen can compute classesNeeded exactly,
    // matching the same formula used in AttendanceScreen.
...
class DashboardViewModel @Inject constructor(
    ...
    val uiState: StateFlow<DashboardUiState> = combine(
        ...
        val todayInt   = LocalDate.now().dayOfWeek.value  // 1=Mon…7=Sun

        val nextClass = slots
            .filter { it.classType != ClassType.BREAK }
            .mapNotNull { slot ->
                val daysUntil = when {
        ...
        DashboardUiState(
            ...
            nextClass         = nextClass,
            hasAnySlots       = slots.any { it.classType != ClassType.BREAK },
